...
class SentryTargetTest extends TestCase
{
    ...
    public function testSentryComponentExtraCallbackIsApplied(): void
    {
        $target = $this->getConfiguredSentryTarget();
    ...
    public function testBothExtraCallbacksAreMergedWithTargetPrecedence(): void
    {
        $target = $this->getConfiguredSentryTarget();
        
        $sentryCallbackExecuted = false;
        ...
        $target->extraCallback = function ($extra, $message) use (&$targetCallbackExecuted) {
            ...
            $extra['target_only'] = 'target_value';
            return $extra;
        };
        ...
        $client->expects($this->once())
            ->method('captureMessage')
            ->willReturnCallback(function (string $message, ?Severity $level = null, ?Scope $scope = null, ?EventHint $hint = null) use (&$capturedScope): ?EventId {
                $capturedScope = $scope;
                return EventId::generate();
            });
        ...
        $this->assertTrue($sentryCallbackExecuted, 'Sentry component callback was not executed');
        $this->assertTrue($targetCallbackExecuted, 'SentryTarget callback was not executed');
        
        // Verify the scope contains the merged data with correct precedence
        $this->assertNotNull($capturedScope, 'Scope should not be null');
    }

    public function testExtraCallbackMergingOrder(): void
    {
        $target = $this->getConfiguredSentryTarget();
        ...
        // Verify execution order: Sentry component first, then SentryTarget
        $this->assertEquals(['sentry', 'target'], $executionOrder);
    }

    public function testOnlySentryComponentCallbackWorks(): void
    {
        $target = $this->getConfiguredSentryTarget();
        ...
        // No target callback set
        $target->extraCallback = null;
    ...
    public function testOnlyTargetCallbackWorks(): void
    {
        $target = $this->getConfiguredSentryTarget();
        
        $callbackExecuted = false;
        
        // No Sentry component callback
